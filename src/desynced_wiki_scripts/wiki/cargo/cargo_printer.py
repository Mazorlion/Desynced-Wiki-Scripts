...
from desynced_wiki_scripts.wiki.cargo.analyze_type import DataClassTypeInfo, ListTypeInfo, TypeInfo
...
class CargoPrinter:
    ...
    def _print_list(
        ...
        if value:
            assert len(value) <= max_length, f"{field_type} max_length is too short. Is {max_length}, need" f" {len(value)}.\n List: {value}"
    ...
    def _print_field(
        ...
        if self.mode == self.Mode.DECLARATIONS:
            return self._print_field_declaration(field_name=field_name, field_type=field_type, suffix=suffix)

        if self.mode == self.Mode.TEMPLATE:
            return [f"|{field_name}{suffix} = " + "{{{" + f"{self.to_camel_case(field_name + suffix)}" + "|}}}"]
    ...
    def _print_field_declaration(self, field_name: str, field_type: TypeInfo, suffix: str = "") -> List[str]:
        if isinstance(field_type.type, type) and issubclass(field_type.type, Enum):
            return [f"|{field_name}{suffix} = String (allowed" f" values={','.join(list(map(lambda x: x.value, field_type.type.__members__.values())))})"]
