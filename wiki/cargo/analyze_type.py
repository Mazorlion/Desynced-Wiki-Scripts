...
class ListTypeInfo(TypeInfo):
    list_options: ListFieldOptions = field(default=ListFieldOptions())
...
class DataClassTypeInfo(TypeInfo):
    ...
    dataclass_options: DataClassFieldOptions = field(default=DataClassFieldOptions())

    def __post_init__(self):
        self.kind = TypeInfo.Kind.DATACLASS


def analyze_type(obj_type: Type) -> TypeInfo:
    ...
    for field_info in fields(obj_type):
        ...
        if hasattr(field_type, "__origin__") and field_type.__origin__ is list:
            ...
            list_field.list_options = get_field_options(f=field_info)
            ...
            dataclass_field: DataClassTypeInfo = analyze_type(field_type)
            dataclass_field.dataclass_options = get_field_options(f=field_info)
            ret.fields[field_info.name] = dataclass_field

        else:
            ret.fields[field_info.name] = TypeInfo(
                type=field_type, options=get_field_options(field_info)
            )
